...
PROJECT_DIR="romer-jvm"
PACKAGE_DIR="org/cequals/Romer"
...
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
	...
	DYNAMIC_LIB_PATH="target/x86_64-unknown-linux-gnu/release/libromer.so"
	...
	DYNAMIC_LIB_PATH="target/x86_64-apple-darwin/release/libromer.dylib"
	...
	DYNAMIC_LIB_PATH="target/aarch64-apple-darwin/release/libromer.dylib"
...
$UNIFFI_BINDGEN_BIN generate bindings/romer.udl --language kotlin -o "$TARGET_DIR" || exit 1

cp "$TARGET_DIR"/"$PACKAGE_DIR"/romer.kt "$BINDINGS_DIR"/"$PROJECT_DIR"/lib/src/main/kotlin/"$PACKAGE_DIR"/ || exit 1
