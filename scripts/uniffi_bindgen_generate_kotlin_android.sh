...
$UNIFFI_BINDGEN_BIN generate bindings/romer.udl --language kotlin --config uniffi-android.toml -o "$BINDINGS_DIR"/"$PROJECT_DIR"/lib/src/main/kotlin || exit 1
...
cp $TARGET_DIR/x86_64-linux-android/release-smaller/libromer.so $JNI_LIB_DIR/x86_64/ || exit 1
cp $TARGET_DIR/armv7-linux-androideabi/release-smaller/libromer.so $JNI_LIB_DIR/armeabi-v7a/ || exit 1
cp $TARGET_DIR/aarch64-linux-android/release-smaller/libromer.so $JNI_LIB_DIR/arm64-v8a/ || exit 1
